...
const client = new MarklyClient();
// Tool: markly_watermark_text
server.tool("markly_watermark_text", "Add a text watermark to an image. Provide an image URL and watermark text. Returns a download URL (valid 30 min).", {
...
server.tool("markly_watermark_logo", "Add a logo/image watermark to an image. Provide image and logo URLs. Returns a download URL (valid 30 min).", {
...
server.tool("markly_batch_watermark", "Watermark multiple images at once. Upload image URLs, apply text watermark, get a ZIP. Supports up to 20 images.", {
...
server.tool("markly_check_usage", "Check your Markly API usage and quota. Shows plan, daily requests, limits, and total images processed.", {}, async () => {
    try {
